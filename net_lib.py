...
def ExtractFeatures(data, window_size, sampling_frequency, dwt_level=4):
    #dwt_level =0 -> there's no such wavelet transform
    if dwt_level !=0:
        wl = WaveletLength(window_size, dwt_level, dec_len)
    else:
        wl = window_size
...
def __getitem__(self, idx):
    ...
    if input_data.shape[1] > padlen:
        ...
        print(f"Data was too trimmed. Len: {input_data.shape[1]}, dim: {input_data.shape}")
        return None, None
...
class Input_Embedding(tf.keras.layers.Layer):
    def __init__(self, seq_len=37, embedding_dim=120, channels=20):
        ...
        self.conv = tf.keras.layers.Conv1D(embedding_dim * channels, 1, groups=channels)
